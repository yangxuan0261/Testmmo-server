...
function RPC.rpc_server_combat (args) -- 都到攻击tid的指令
	...
	damage = skynet.call (agent, "lua", "cmd_combat_melee_damage", user.character.id, damage) -- 通知agent受伤

	return { target = tid, damage = damage } -- 返回被攻击的角色id及伤害值
end

function CMD.cmd_combat_melee_damage (attacker, damage) -- 攻击我的角色(attacker)，及伤害值damage
